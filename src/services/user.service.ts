...
export class UserService {
  ...
  public getUser = async (userId: string): Promise<UserInfo> => {
    try {
      ...
      const contacts = Array.isArray(userInfo.contacts)
        ? userInfo.contacts
        : [];
      const demographics = Array.isArray(userInfo.demographics)
  ...
  public saveUserDemographics = async ({
    ...
    try {
      const userRepo = DBConnection.getRepository(UserEntity);
      const userDemographicsRepo = DBConnection.getRepository(
      ...
      if (existingDemographics) {
        await userDemographicsRepo.update(
          existingDemographics.id,
          userDemographics,
        );
      } else {
